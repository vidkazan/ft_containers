...
//                print(1,_root);
            ...
            void    balanceAfterDelete(node* deleted)
            {
                if(_debug)
                ...
                cout << "balance after delete: balance at " << parent->key << "\n";
    ...
    //                     case 1.2: left Black - merge
            ...
            void    merge(node* ptr) {
                if(_debug)
                    cout << "\nmerge "  << "\n";
                if(ptr->left->key) {
                    cout << "merge left: " << ptr->key << " and " << ptr->left->key << "\n";
                    ptr->left->color = 1;
                    ptr->right->key = ptr->key;
                    if(!ptr->right->right)
                        ptr->right->right = createNullNode(ptr->right);
                    ptr->key = 0;
                    ptr->right->left = ptr->left;
                    ptr->left = NULL;
                    ptr->right->left->parent = ptr->right;
                    if(!ptr->parent)
                        _root = ptr->right;
                    _root->parent = NULL;
                    this->print(1,_root);
                }
                else if(ptr->right->key) {
                    cout << "merge right: " << ptr->key << " and " << ptr->right->key << "\n";
                    ptr->left->color = 1;
                    ptr->left->key = ptr->key;
                    if(!ptr->left->left)
                        ptr->left->left = createNullNode(ptr->left);
                    if(!ptr->left->right)
                        ptr->left->right = createNullNode(ptr->left);
                    ptr->key = 0;
                    ptr->right->left = ptr->left;
    //                    ptr->left = NULL;
                    ptr->right->left->parent = ptr->right;
...
//                cout << "create root node " << key <<"\n";
            ...
            void    erase(int key)
            {
                ...
                if (ptr->left->key && ptr->right->key) {
                    ...
                    balanceAfterDelete(replaceSuccessor(ptr));
                } else {
